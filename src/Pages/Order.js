...
import { AddCart, AddOrder } from '../store'



function OrderOptionSubC({Fdata, setColorOpt}){

   return Fdata.color.map((item) => {
      return (
    <li key={item} onClick={() => {setColorOpt(item);} }>
      <p>
        <img src={`./Images/ordermade/${item}.png`} alt={item}/>
      </p>
      <p className='orderName'>{item}</p>
    </li>
    )
    })
}

function OrderOptionSubO({Fdata,setOption,setOptPrice, orderType}){
  return Fdata.option.kind.map((item,index) => {
    return(
      <li key={index} onClick={() => {setOption(item);setOptPrice(Fdata.option.price[index])}}>
      <p><img src={`./Images/ordermade/${orderType}_option_${item}.png`} alt={item}/></p>
      <p className='orderName'>{item}</p>
      <p className='orderPrice'>{Fdata.option.price[index]}</p>
    </li>
    )
  })
}

function OrderOptionMain({Data,setOrderType,setColorOpt,setFdata,setOption}){
  ...
  return Data.map((item,index) => {
    return(
    <li key={index} onClick={() => {Setting(item)}}>
      <p><img src={`./Images/ordermade/${item.name}_default.png`} alt=''/></p>
      <p className='orderName'>{item.name}</p>
      <p className='orderPrice'>{item.price}</p>
    </li>
    )
  })
}
...
function OrderCont({phase,setPhase,Data}){
  const [check,setCheck] = useState(false)
  const [initial,setInitial] = useState(0)
  const [inputW,setInputW] = useState(false)
  const [orderType,setOrderType] = useState('wallet')
  const [colorOpt,setColorOpt] = useState('default')
  const [option, setOption] = useState('default')
  const [Fdata,setFdata] = useState(Data[0])
  const [Sign,setSign] = useState('Nosign')
  const [optPrice,setOptPrice] = useState(0)

  let dispatch = useDispatch()

  let CartData = {
    name : `${orderType}_${colorOpt}_${option}_${Sign}`,
    picture : `./Images/ordermade/${orderType}_${colorOpt}.png`,
    amount : 1,
    optstype : option,
    optscolor : colorOpt,
    price : (Fdata.price + optPrice),
    type : "order",
    total : (Fdata.price + optPrice),
    sign : $('#initcont').val()
  }
 ...
 // 상시 Function
  $(function(){
    ...
    function Checking(){
    if (check){
      $('p.ordernext').fadeIn(500)
    }
    else{
      $('p.ordernext').fadeOut(500)
    }
  ...
  function Inputing(){
      if (inputW){
        $('input#initcont').attr("disabled", false)
      }
      else{
        $('input#initcont').attr("disabled", true).val('')
      }
    }
  Inputing()
  })
// 기타 function
  function InputChange(x){
    if(x.value.length > 6){
      x.value = x.value.substr(0,6)
    }
  }
  function CheckOn(x){
    let check = $(x).is(':checked')
    console.log(check)
    if (check){
      setCheck(check)
    }
  ...
  function WarnInit(){
    if (initial > 6)
    return(
      <p className='overMax'>6자 이하로 입력해주세요</p>
    )
    return (
      <p className='underMax'>각인은 최대 6자까지 가능합니다.</p>
    )
  }
  function ColorTitle(){
    if(orderType === "acc"){
  ...
  if (phase === 1){
    ...
    <div id='introduction'>
      <div id='intro'>
          <dl>
            <dt>나만의 가죽제품을 만들어보세요</dt>
            <dd>세상에 하나 뿐인 특별한 제품</dd>
            <dd>자유로운 커스터마이징</dd>
          </dl>
      </div>
      <div id='ordercaution'>
        <h3>주의사항</h3>
        <dl className='warranty'>
          <dt>Order제품 보증규정</dt>
          <dd>제1조(목적)<br/>
이 약관은 hevitz 회사(전자상거래 사업자)가 운영하는 hevitz 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리의무 및 책임사항을 규정함을 목적으로 합니다.<br/>

※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」<br/>

제2조(정의)<br/>
①“몰” 이란 hevitz 회사가 재화 또는 용역(이하 “재화등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.<br/>
...
③ “몰”은 전자상거래등에서의소비자보호에관한법률, 약관의규제에관한법률, 전자거래기본법, 전자서명법, 정보통신망이용촉진등에관한법률, <br/>방문판매등에관한법률, 소비자보호법 등 관련법을 위배하지 않는 범위에서 이 약관을 개정할 수 있습니다.<br/>
...
③제공된 개인정보는 당해 이용자의 동의없이 목적외의 이용이나 제3자에게 제공할 수 없으며, 이에 대한 모든 책임은 　hevitz몰　이 집니다. 다만, 다음의 경우에는 예외로 합니다.<br/>
...
⑦ “몰” 또는 그로부터 개인정보를 제공받은 제3자는 개인정보의 수집목적 또는 제공받은 목적을 달성한 때에는 당해 개인정보를 지체없이 파기합니다.
       ...
       <form id='agreement'>
        <fieldset>
          <input id='agreck' type="checkbox" onClick={(e) => {CheckOn(e.target)}}/>
          <label htmlFor='agreck'>상기 내용에 동의합니다.</label>
          <p className='ordernext'>
          <button className='agreecheck' onClick={(e) => {setPhase(2);}}>Order Made Now</button>
          </p>
        </fieldset>
       </form>
    </div>
  ...
  <>
    <div className='orderShow'>
          <p><img src={`./Images/ordermade/${orderType}_${colorOpt}.png`} alt={orderType + colorOpt}/></p>
          <div className='orderOption'>
            <h3>제품 선택</h3>
            <ul id='orderkind'>
              <OrderOptionMain Data = {Data} setOrderType = {setOrderType} setColorOpt={setColorOpt} setFdata={setFdata} setOption ={setOption}  />
            </ul>
            <ColorTitle/>
            <ul id='ordercolor'>
              <OrderOptionSubC Fdata = {Fdata} setColorOpt = {setColorOpt} />
            </ul>
          </div>
    </div>
    <div className='orderShow'>
        <p><img src={`./Images/ordermade/${orderType}_option_${option}.png`} alt={orderType + option}/></p>
        <div className='orderOption'>
          <h3>옵션선택</h3>
          <ul id='detailoption'>
            <OrderOptionSubO Fdata = {Fdata} setOption ={setOption} setOptPrice = {setOptPrice} orderType ={orderType} />
          </ul>
        </div>
    </div>
    <div className='orderShow'>
        <p><img src='./Images/ordermade/initial.png' alt='Initial'/></p>
        <div className='orderOption'>
          <h3>각인유무</h3>
          <div id='signinitial'>
            <div id='noinitial'>
            <input type="radio" defaultChecked={true} name='sign' id='signoff' onClick={() => {setInputW(false);setSign('NoSign')}}/>
            <label htmlFor='signoff'>각인없음</label>
            </div>
            <div id='yesinitial'>
            <input type="radio" name='sign' id='signon' onClick={() => {setInputW(true); setSign('YesSign');}}/>
            <label htmlFor='signon'>각인있음</label>
            <input id='initcont' type='text' disabled={true} onInput={(e) => {setInitial(e.target.value.length,InputChange(e.target))}}/>
            <WarnInit/>
            </div>  
          </div>
        </div>
    ...
    <div id='orderresult'>
      <div className='orderStatus'>
        <dl>
          <dt>제품사양</dt>
          <dd>{`${orderType}_${colorOpt}_${option}_${Sign}`}</dd>
        </dl>
        <dl>
          <dt>제품가격</dt>
          <dd>{Fdata.price + optPrice +'원'}</dd>
        </dl>
      </div>
      <div className='orderBtn'>
        ...
        <button onClick={() => {
          dispatch(AddCart(CartData)); window.alert('장바구니에 추가되었습니다!')
        }}>장바구니</button>
        <button onClick={() => {window.alert('이 기능은 아직 준비중입니다. ')}}>바로구입</button>
      </div>
    </div> 
  </>
    )
...
function Order({OrderData}){
  ...
  return(
    <article id="order">
      <h2>OrderMade</h2>
      <div id="orderwrap">
        <OrderCont phase = {phase} setPhase = {setPhase} Data = {orderData}/>
      </div>
    </article>
  )
}
